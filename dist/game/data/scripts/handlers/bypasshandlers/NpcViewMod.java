...
public class NpcViewMod implements IBypassHandler
{
	...
	public boolean useBypass(String command, L2PcInstance activeChar, L2Character bypassOrigin)
	{
		...
		if (!st.hasMoreTokens())
		{
			_log.warn("Bypass[NpcViewMod] used without enough parameters.");
			return false;
		}
	...
	public static void sendNpcView(L2PcInstance activeChar, L2Npc npc)
	{
		...
		html.setFile(activeChar, activeChar.getHtmlPrefix(), "data/html/mods/NpcView/Info.htm");
	...
	public static void sendNpcDropList(L2PcInstance activeChar, L2Npc npc, DropListScope dropListScope, int page)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			else if (dropItem instanceof GroupedGeneralDropItem)
			{
				...
				else
				{
					...
					{
						final L2Item item = ItemData.getInstance().getTemplate(generalDropItem.getItemId());
		...
		String html = HtmCache.getInstance().getHtm(activeChar, activeChar.getHtmlPrefix(), "data/html/mods/NpcView/DropList.htm");
		if (html == null)
		{
			_log.warn(NpcViewMod.class.getSimpleName() + ": The html file data/html/mods/NpcView/DropList.htm could not be found.");
			return;
		}
	...
	{
		final L2Item item = ItemData.getInstance().getTemplate(dropItem.getItemId());
		sb.append("<table width=332 cellpadding=2 cellspacing=0 background=\"L2UI_CT1.Windows.Windows_DF_TooltipBG\">");
		sb.append("<tr><td width=32 valign=top>");
