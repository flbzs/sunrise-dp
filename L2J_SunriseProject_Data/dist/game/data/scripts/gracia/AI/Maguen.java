...
public final class Maguen extends AbstractNpcAI
{
	 ...
	 *           int c = c_info == null ? 0 : c_info.getSkill().getAbnormalLvl(); final int r = r_info == null ? 0 : r_info.getSkill().getAbnormalLvl(); if ((b == 3) && (c == 0) && (r == 0)) { showOnScreenMsg(player, NpcStringId.ENOUGH_MAGUEN_PLASMA_BISTAKON_HAVE_GATHERED, 2, 4000);
	 *           player.getEffectList().stopSkillEffects(B_PLASMA1.getSkillId()); npc.setTarget(player); npc.doCast((getRandom(100) < 70) ? B_BUFF_1.getSkill() : B_BUFF_2.getSkill()); maguenPetChance(player); startQuestTimer("END_TIMER", 3000, npc, player); } else if ((b == 0) && (c == 3) && (r ==
	 *           0)) { showOnScreenMsg(player, NpcStringId.ENOUGH_MAGUEN_PLASMA_COKRAKON_HAVE_GATHERED, 2, 4000); player.getEffectList().stopSkillEffects(C_PLASMA1.getSkillId()); npc.setTarget(player); npc.doCast((getRandom(100) < 70) ? C_BUFF_1.getSkill() : C_BUFF_2.getSkill());
	 *           maguenPetChance(player); startQuestTimer("END_TIMER", 3000, npc, player); } else if ((b == 0) && (c == 0) && (r == 3)) { showOnScreenMsg(player, NpcStringId.ENOUGH_MAGUEN_PLASMA_LEPTILIKON_HAVE_GATHERED, 2, 4000); player.getEffectList().stopSkillEffects(R_PLASMA1.getSkillId());
	 *           npc.setTarget(player); npc.doCast((getRandom(100) < 70) ? R_BUFF_1.getSkill() : R_BUFF_2.getSkill()); maguenPetChance(player); startQuestTimer("END_TIMER", 3000, npc, player); } else if ((b + c + r) == 3) { if ((b == 1) && (c == 1) && (r == 1)) {
	 *           player.getEffectList().stopSkillEffects(B_PLASMA1.getSkillId()); player.getEffectList().stopSkillEffects(C_PLASMA1.getSkillId()); player.getEffectList().stopSkillEffects(R_PLASMA1.getSkillId()); showOnScreenMsg(player,
	 *           NpcStringId.THE_PLASMAS_HAVE_FILLED_THE_AEROSCOPE_AND_ARE_HARMONIZED, 2, 4000); SkillHolder skillToCast = null; switch (getRandom(3)) { case 0: skillToCast = (getRandom(100) < 70) ? B_BUFF_1 : B_BUFF_2; break; case 1: skillToCast = (getRandom(100) < 70) ? C_BUFF_1 : C_BUFF_2; break;
	 *           case 2: skillToCast = (getRandom(100) < 70) ? R_BUFF_1 : R_BUFF_2; break; } if (skillToCast != null) { npc.setTarget(player); npc.doCast(skillToCast.getSkill()); } maguenPetChance(player); startQuestTimer("END_TIMER", 3000, npc, player); } else { showOnScreenMsg(player,
	 *           NpcStringId.THE_PLASMAS_HAVE_FILLED_THE_AEROSCOPE_BUT_THEY_ARE_RAMMING_INTO_EACH_OTHER_EXPLODING_AND_DYING, 2, 4000); player.getEffectList().stopSkillEffects(B_PLASMA1.getSkillId()); player.getEffectList().stopSkillEffects(C_PLASMA1.getSkillId());
	 *           player.getEffectList().stopSkillEffects(R_PLASMA1.getSkillId()); } } else { startQuestTimer("END_TIMER", 1000, npc, player); } npc.setDisplayEffect(4); return super.onSpellFinished(npc, player, skill); }
	 */
